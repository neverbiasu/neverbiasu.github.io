...
- **V3.0-vpred** 则采用 velocity 预测（v 参数化），展示出更强的组合理解能力，但最初伴随着严重问题，包括灾难性遗忘、领域偏移、颜色过饱和以及因零终端信噪比（zero terminal SNR）实现失误而导致的色板崩溃。
...
v 参数化的主要问题在于，由于 zero terminal SNR 导致的方差估计错误。这一误差使模型在初期过度移除噪声，过早高估图像清晰度，从而导致整个去噪过程中颜色饱和度过高或不真实。
