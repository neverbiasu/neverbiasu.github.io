...
- **V3.0-vpred** 则采用速度预测（v 参数化），展示出更强的组合理解能力，但最初伴随着严重问题，包括灾难性遗忘、领域偏移、颜色过饱和以及因零终端信噪比（zero terminal SNR）实现失误而导致的色板崩溃。
...
v 参数化的主要问题在于，由于零终端 SNR 导致的方差估计错误。这一误差使模型在初期过度移除噪声，过早高估图像清晰度，从而导致整个去噪过程中颜色饱和度过高或不真实。

尽管存在这些挑战，v 参数化仍带来了显著的组合改进。例如，该模型能够成功解释复杂的方向性提示（例如"左侧是黑色，右侧是红色"），这一成就此前的 epsilon 模型难以实现。

（epsilon 模型，然而，在处理无法仅用随机性解释的组合时也展现了显著改进。见下文说明。）
...
3. **使用简单提示进行 FFT 分析**：对模型中间潜变量进行 Fast Fourier Transform 分析，以研究低频信号变化。

![v3.0-vpred 简单提示推理结果](/assets/images/reprints/illustrious/v3.0-3.5/1.png)
*模型部分遵循提示："图中有两个女孩。左侧女孩有红发和海黑色眼睛，右侧女孩有蓝发和火红宝石眼"，成功率较高，但默认输出存在颜色过饱和现象。*

简单提示推理结果还展示了 v3.0 模型的一些有趣现象。

v3.0 Epsilon 同样显示出显著改进，这一点我仍然解释得不够充分。

> "图中描绘了两位女孩，左侧女孩有红发、黄眼和蓝裙；右侧女孩则拥有蓝调银发、黄耳环、蓝色瞳孔和银色礼服。"
> 采样器：Euler，调度类型：Normal，CFG scale：7.5

![v3.0-epsilon 推理结果](/assets/images/reprints/illustrious/v3.0-3.5/2.png)
*令人惊讶的是，epsilon 模型有时表现良好，这里是 25 次中有 23 次成功。*

![v3.0-vpred 推理结果](/assets/images/reprints/illustrious/v3.0-3.5/3.png)
*模型在默认情况下颜色过饱和。*

![v3.5-vpred 推理结果](/assets/images/reprints/illustrious/v3.0-3.5/4.png)
*模型呈现中间色调的颜色分离效果。*
...
![v3.0-vpred 推理结果](/assets/images/reprints/illustrious/v3.0-3.5/5.png)
*模型无法生成稳定输出，暴露出颜色过饱和问题。*

![社区驱动的 v 参数化模型结果](/assets/images/reprints/illustrious/v3.0-3.5/6.png)
*模型同样存在颜色过饱和，且未能很好地遵循提示。*

![其他流行的 epsilon 预测模型结果](/assets/images/reprints/illustrious/v3.0-3.5/7.png)
*该提示揭示了模型的一些问题。*

![v3.0-epsilon 生成结果](/assets/images/reprints/illustrious/v3.0-3.5/8.png)
*模型能够生成相对更稳定的风格化输出，在所有结果中表现较好。*

![Flux.1-Schnell 生成结果](/assets/images/reprints/illustrious/v3.0-3.5/9.png)
*模型在遵循提示方面表现得非常出色，但缺乏风格化效果。*

![Illustrious-v3.5-vpred 生成结果](/assets/images/reprints/illustrious/v3.0-3.5/10.png)
*模型展示出与 Flux 类似的稳定生成效果，但失去了 v3.0-epsilon 所特有的风格化特质。我注意到 Flux 的提示遵循"明显好且稳健"，而 v3.5-vpred 的提示遵循"可能较为局限"——高度依赖提示的格式。*
...
![色彩丰富度指标](/assets/images/reprints/illustrious/v3.0-3.5/14.png)  
...
![比较结果](/assets/images/reprints/illustrious/v3.0-3.5/15.png)
